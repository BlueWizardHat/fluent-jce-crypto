...
public class TwofishFactoryTest {
	private SymmetricEncryptionEngine[] engines = {
			TwofishFactory.usingTwofishCtr(),
			TwofishFactory.usingTwofishCfb(),
			TwofishFactory.usingTwofishCbc(),
			TwofishFactory.usingTwofishEcb()
		};
	...
	public static void classSetup() throws Exception {
		...
		testKeySize = Math.min(256, Cipher.getMaxAllowedKeyLength("Twofish"));
		System.out.println("Running TwofishFactory tests with keysize " + testKeySize);
	}
	...
	public void testKeyBasedEncryption() {
		for (SymmetricEncryptionEngine engine : engines) {
			// Setup
			Key key = TwofishFactory.generateKey(testKeySize);
